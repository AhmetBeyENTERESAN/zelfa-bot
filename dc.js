...
client.on("ready", async () => {
  let sesliKanalID = client.channels.cache.get("1043984455305535589");
  if (sesliKanalID)
    sesliKanalID.join()
...
client.on("message", async msg => {
  ...
  client.on("message", async mesaj => {
    ...
    host: '217.195.202.234',
        ...
        const embed = new Discord.MessageEmbed()
        .setColor("#000000")
        .setTitle("**ZelfaRPG Sunucu İstatistikleri**")
        .setThumbnail(mesaj.guild.iconURL({dynamic: true, type: 'png'}))
        .addField('Sunucu IP: ', "```217.195.202.234```")
...
setInterval (function () {
    ...
    host: '217.195.202.234',
		...
		client.user.setActivity('IP: 217.195.202.234:22003',{type: "WATCHING"})
		}else{
		client.user.setActivity(`Oyuncu : ${state.raw.numplayers}/${state.maxplayers}`)
		}
    });
}, 3000); 
